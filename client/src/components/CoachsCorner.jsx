...
import { MessageSquare, Zap, TrendingUp, Star, Phone, Trash2 } from 'lucide-react';
...
export default function CoachsCorner({ people }) {
  ...
  const [producer, setProducer] = useState(producers[0]?.name || '');
  ...
  async function handleSave() {
    ...
    try {
      await fetch('/api/coaching', {
        ...
        body: JSON.stringify({ producer, date, notes }),
      });
      setNotes('');
      fetchNotes();
    } catch {}
  ...
  return (
    <div className="coach-page">
      ...
      {/* Manual coaching notes — live now */}
      <div className="coaching-form">
        <div className="coaching-form-title">Add Coaching Note</div>
        <div className="coaching-form-row">
          <div className="form-group" style={{ flex: 1 }}>
            <label className="form-label">Producer</label>
            <select className="form-select" value={producer} onChange={e => setProducer(e.target.value)}>
      ...
      {coachNotes.length > 0 && (
        <div>
          <div className="section-title">Saved Coaching Notes</div>
            ...
            {coachNotes.map(note => (
              <div key={note.id} className="coaching-note-card">
                <div className="coaching-note-meta">
                  ...
                  <div className="coaching-note-date">{formatDate(note.createdAt)}</div>
                </div>
      ...
      {coachNotes.length === 0 && (
        <div className="empty-state" style={{ padding: 24 }}>
          ...
          <p>No coaching notes yet.<br />Add your first note above.</p>
        </div>
      )}
    </div>
  );
